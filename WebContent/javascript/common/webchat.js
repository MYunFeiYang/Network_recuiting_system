...
var socket;
function init() {
    socket = new WebSocket("ws://localhost:80/init");
}
$(function() {
    //初始化加载listen方法
    listen();
...
function emit() {
    ...
    $("#show_content").append("<kbd style='color: #" + "CECECE" + ";float: right; font-size: " + 12 + ";'>" + text +  "</kbd><br/>");
    //将消息文本框清空
    $("#msg").val("");
}
...
function listen() {
    ...
    socket.onopen = function() {
        $("#show_content").append("<kbd>Welcome!</kbd></br>");
    };
    //收到消息时触发
    ...
    socket.onclose = function(evt) {
        $("#show_content").append("<kbd>" + "Close!" + "</kbd></br>");
    }
    //连接错误时触发
    socket.onerror = function(evt) {
        $("#show_content").append("<kbd>" + "ERROR!" + "</kbd></br>");
    }
}
//按下回车键时触发发送消息方法
